...
- 生成的 `files.download` commands 使用 [`src/models/configs.constants.ts`](src/models/configs.constants.ts) 中的 `CONFIG_BASE_URL`。修改下载来源时，同步检查生成结果和 README 说明，不要在文档中重复维护具体 URL。
